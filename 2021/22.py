...
minn,maxx=min([min([min(x) for x in y]) for y in queue]),max([max([max(x) for x in y]) for y in queue])
cubes=[]
...
while queue:
    ...
    for j,cube2 in enumerate(cubes):
        ...
        for i in range(3):
            ...
            p,q = cube1[i]
            r,s = cube2[i]
            if p==r and q==s: #both edges exactly match for this axis (already split along this axis previously)
                #print("continue")
                continue
