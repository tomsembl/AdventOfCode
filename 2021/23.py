...
iterations=0
while True:
    try: boxState, hallState, energy, history = queue.pop()
    except IndexError: break
    iterations+=1
    if not iterations % 10_000_000: print(iterations,len(queue))
        ...
        if not validateHome(boxState, i, amphipod):
            ...
            for x in distances[i]:
                ...
                if validatePath(boxState,hallState,path):
                    queue.append([boxState[:i]+[0]+boxState[i+1:], hallState[:x-8]+[amphipod]+hallState[x-7:], energy+cost*costMultiplier(amphipod), history+[[boxState,hallState,energy]]])
            #go home -> home
            ...
            if validatePath(boxState,hallState,path):
                ...
                queue.append([newBoxState, hallState, energy+cost*costMultiplier(amphipod), history+[[boxState,hallState,energy]]])
    for i2,amphipod2 in enumerate(hallState):
        ...
        if validatePath(boxState,hallState,path):
            queue.append([boxState[:home]+[amphipod2]+boxState[home+1:], hallState[:i2]+[0]+hallState[i2+1:], energy+cost*costMultiplier(amphipod2), history+[[boxState,hallState,energy]]])
        # if not validateHome(boxState, i, amphipod):
        #     for x in distances[i]:
        #         path, cost = distances[i][x]
        #         if validatePath(boxState,hallState,path):
