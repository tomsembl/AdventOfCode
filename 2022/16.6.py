...
while queue:
    ...
    if totalChecked % 1000 == 0 and queue:
        if len(queue) >30000000:
            stopQueuing = True
        f = open("c:/temp/demofile3.txt", "w")
        f.write(f"maxFlow:{maxFlow}. {totalChecked} iterations. queueLength={len(queue)}. queue[0]: {queue[0]}")
        f.close()
        print(f"maxFlow:{maxFlow}. {totalChecked} iterations. queueLength={len(queue)}. queue[0]: {queue[0]}")
        #print(f"queue end: {queue[-1]}")
    totalChecked += 1
    if stopQueuing: continue
    ...
    #if one is None, get the candidates for the other
    if noneExists:
        i = notNoneIndex
        ...
        candidates[i].sort(key=lambda x: valves[x]["flowRate"]*(REMAININGMINUTES-valves[nodes[i]]["distances"][x]), reverse=True)
    ...
    #normal 2 nodes
        ...
        for candidate0 in candidates[0]:
            ...
            candidates1.sort(key=lambda x: valves[x]["flowRate"]*(REMAININGMINUTES-valves[nodes[i]]["distances"][x]), reverse=True)
