...
while b[i]!=99:
    ...
    if op==4: 
        if x==10: grid.append([])
        else: grid[-1].append(x)#{35:"#",46:"."}[x] if x in [35,46] else "X")
        # print(x)
    if op==5 and x!=0:
        jumped = True
        i=y
...
def getNeighs(x,y):
    neighs = [[x+xx,y+yy] for xx,yy in dirs]
    return neighs

intersections=[]
for j,y in enumerate(grid):
    for i,x in enumerate(y):
        if all([x==35, 0<i<(w-1), 0<j<(h-1)]):
            if all([grid[jj][ii]==35 for ii,jj in getNeighs(i,j)]):                
                intersections.append((i,j))
for line in grid:
    print("".join(map(chr,line)))
print(sum([x*y for x,y in intersections]))#part1

def getNeigh(x,y,dir):
    dx,dy = dirs[dir]
    xx,yy = x+dx, y+dy
    return (xx,yy)
...
[[[startDir:=x,start:=(i,j)] for i,x in enumerate(y) if x not in [35,46]] for j,y in enumerate(grid)]
