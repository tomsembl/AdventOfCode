...
def getFunctionsAndRoutine():
    path = pathFind()
    pathStr = ",".join([str(x) for x in path])
    for ax in range(1,21):
        for bx in range(1,21):
            for cx in range(1,21):
                breakFlag = False
                mainRoutine = pathStr[::]
                xList=[ax,bx,cx]
                functions={}
                for i,x in enumerate("ABC"):
                    ...
                    while True: 
                        try:
                            ...
                            break
                        j+=1
                    xStr = mainRoutine[j:j+xList[i]]
                    if xStr[-1]==",": 
                        breakFlag = True
                        break
                    mainRoutine = mainRoutine.replace(xStr,x)
                    functions[x]=xStr
                if len(functions) == 3:
                    if functions["A"] == 'R,8,R,8' and functions["B"] == 'R,4,R,4,R,8':
                        print()
                if any([any([z in zz for zz in functions.values()]) for z in "ABC"]): continue
                if len(mainRoutine) > 20: continue
                if all([not z in mainRoutine for z in "LR0123456789"]):
                    return functions.values(),mainRoutine
                
functions,mainRoutine = getFunctionsAndRoutine()
functions = [x+"\n" for x in functions]
mainRoutine = mainRoutine+"\n"
print(functions,mainRoutine)
result = computer(2,[ord(char) for char in mainRoutine+"".join(functions)+"n\n"])
