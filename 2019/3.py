...
b=[[[y[0],int(y[1:])] for y in x.split(",")] for x in a.splitlines()]
dic={}
collisions=set()
for i,line in enumerate(b):
    ...
    for dir, dist in line:
        ...
        for z in range(dist):
            coord=(x+xx*z,y+yy*z)
            if coord in dic:
                if dic[coord]!=i: collisions.add(coord)
            else: dic[(x+xx*z,y+yy*z)]=i
        x,y=(x+xx*dist,y+yy*dist)
manhattans=[]
for x,y in collisions: manhattans.append(abs(x)+abs(y))
for x in sorted(manhattans):
    if x!=0: 
        print(x) #part 1
        break
...
for i,line in enumerate(b):
    ...
    for dir, dist in line:
        ...
        for z in range(dist):
            coord=(x+xx*z,y+yy*z)
            if coord in dic:
                if dic[coord][0]!=i: collisions.add(dic[coord][1]+n)
            else: dic[(x+xx*z,y+yy*z)]=(i,n)
            n+=1
        x,y=(x+xx*dist,y+yy*dist)
for x in sorted(collisions):
    if x!=0: 
        print(x) #part2
        break