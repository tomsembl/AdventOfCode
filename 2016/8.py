...
for instr in b:
    if "rect" in instr:
        width,height = [int(x) for x in instr[1].split("x")]
        for j in range(height):
            for i in range(width):
                matrix[j][i] = 1
    elif "row" in instr:
        row,shift = int(instr[2][2:]), int(instr[-1])
        matrixCopy=[y[::] for y in matrix[::]]
        for i in range(w):
            matrix[row][i] = matrixCopy[row][(i-shift)%w]
    elif "column" in instr:
        col,shift = int(instr[2][2:]), int(instr[-1])
        matrixCopy=[y[::] for y in matrix[::]]
        for j in range(h):
            matrix[j][col] = matrixCopy[(j-shift)%h][col]
    
print(sum([sum(y)  for y in matrix])) #part 1
for y in matrix:
    print("".join([" █"[x] for x in y])) #part 2
print()
