a="""aczupnetwp-mfyyj-opalcexpye-977[peyac]
...
for room in b:
    ...
    checksum = room[end:][1:-1]
    set_ = set([char for char in name if char!="-"])
    list_ = sorted(list(set_),key=lambda x: (-name.count(x),x))
    if checksum == "".join(list_[:5]):
        valid+=sectorID
    shift = sectorID%26
    decoded = "".join([x if x=="-" else (alpha*2)[alpha.index(x)+shift] for x in name])
    print(decoded)
    if "north" in decoded:
