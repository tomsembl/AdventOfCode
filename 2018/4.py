a="""[1518-05-29 00:00] Guard #1151 begins shift
...
for x in b:
    ...
    if "#" in x:
        ...
        if "wakes up" in x: guards[guard][monthDay][(hour,minute)] = 1
        else: guards[guard][monthDay][(hour,minute)] = 0

guardMinuteTotals={guard:{x:0 for x in range(60)} for guard in guards}
...
sleepiestMinute = max(guardMinuteTotals[sleepiestGuard],key=lambda x: guardMinuteTotals[sleepiestGuard][x])
print(sleepiestGuard*sleepiestMinute) #part1
