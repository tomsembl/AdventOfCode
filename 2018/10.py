a="""position=<-31065, -31102> velocity=< 3,  3>
...
#a=test
b=[[int(x.split("<")[1].split(",")[0]),  int(x.split(", ")[1].split(">")[0]),  int(x.split("<")[2].split(",")[0]),  int(x.split(", ")[2].split(">")[0])] for x  in a.splitlines()]
minArea = 10**20
minZ, minC, mins = None, None, None
for z in range(20000):
    c = [[x+xx*z,y+yy*z] for x,y,xx,yy in b]
    minX = min([x[0] for x in c])
    maxX = max([x[0] for x in c])
    minY = min([x[1] for x in c])
    maxY = max([x[1] for x in c])
    width = maxX - minX
    height = maxY - minY
    area = width*height
    if area < minArea: 
        ...
        minZ = z
        minC = c
        mins = [minX,maxX,minY,maxY]

minX,maxX,minY,maxY = mins
matrix = [[" " for x in range(maxX-minX+1)] for y in range(maxY-minY+1)]
for x,y in minC:
