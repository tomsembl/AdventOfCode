...
frs: qnr lhk lsr"""
#a=test
b={x.split(": ")[0]: set(x.split(": ")[1].split()) for x in a.splitlines()}
for x in list(b):
    for y in b[x]:
        b.setdefault(y,set()).add(x)
for x in b:
    print(x,b[x])
    
def traverseAndCount(b,node):
    #node = list(b)[0]
    queue = [node]
    seen = set()
    while queue:
        ...
        seen.add(node)
        nexts = [x for x in b[node] if x not in seen]
        print(node,nexts)
        queue.extend(nexts)
    return len(seen)
...
def visNetwork():
    ...
    net.show("network.html", True, False)
#visNetwork()

#find the 3 middle connections and enter them below manually V
connectionsToRemove = {"thl":"nmv", "mbq":"vgk", "fxr":"fzb"}
for x in connectionsToRemove:
    b[x].remove(connectionsToRemove[x])
    b[connectionsToRemove[x]].remove(x)
sides = set()
for x in list(connectionsToRemove.keys()) + list(connectionsToRemove.values()):
    sides.add(traverseAndCount(b,x))
print(sides)