a="""Game 1: 20 green, 3 red, 2 blue; 9 red, 16 blue, 18 green; 6 blue, 19 red, 10 green; 12 red, 19 green, 11 blue
...
games=[[[z.split() for z in y.split(", ")] for y in x.split(": ")[1].split(";")] for x in a.splitlines()]
maxColours = {"red":12,"green":13,"blue":14}
def isValid(game):
    return all([all([int(qty) <= maxColours[colour] for qty,colour in round]) for round in game])
print(sum([i+1 for i,game in enumerate(games) if isValid(game)]))#part 1

powerSum = 0
for game in games:
    power = 1
    gameMaxColours = {"red":0,"green":0,"blue":0}
    for round in game:
        for qty,colour in round:
            if int(qty) > gameMaxColours[colour]:
                gameMaxColours[colour] = int(qty)
    for qty in gameMaxColours.values():
        power *= qty
    powerSum += power
print(powerSum)