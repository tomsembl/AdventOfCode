...
for line in b:
    buttonA = line.split("\n")[0].split(": ")[1]
    buttonAXY = [int(buttonA.split(", ")[0].split("+")[1]) , int(buttonA.split(", ")[1].split("+")[1])]
    buttonB = line.split("\n")[1].split(": ")[1]
    buttonBXY = [int(buttonB.split(", ")[0].split("+")[1]) , int(buttonB.split(", ")[1].split("+")[1])]
    prize = line.split("\n")[2].split(": ")[1]
    prizeXY = [int(prize.split(", ")[0].split("=")[1]) , int(prize.split(", ")[1].split("=")[1])]
    machines.append([buttonAXY,buttonBXY, prizeXY])

for buttonA, buttonB, prizeCoords in machines:
    px,py = prizeCoords
    ax,ay = buttonA
    bx,by = buttonB
    
    maxPressesRequired = 0
    maxPressesRequired = max([px//ax, py//ay, px//bx, py//by]) + 1
    
    bestCost = -1
    for na in range(maxPressesRequired):
        for nb in range(maxPressesRequired):
            x = (ax * na) + (bx * nb)
            y = (ay * na) + (by * nb)
            totalCost = costA * na + costB * nb
            if x > px or y > py:
                break
            if x == px and y == py:
                if bestCost == -1 or totalCost < bestCost:
                    bestCost = totalCost
                    break
    if bestCost == -1:
        continue
...
def cramersRule(coord1,coord2): return coord1[0] * coord2[1] - coord1[1] * coord2[0]
...
for buttonA, buttonB, prizeCoords in machines:
    ...
    divisor = cramersRule(buttonA,buttonB)
    na = cramersRule(prizeCoords,buttonB)
    nb = cramersRule(buttonA,prizeCoords)
    if na % divisor == 0 and nb % divisor == 0: 
        total +=  na//divisor * costA + nb//divisor * costB
