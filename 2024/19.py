...
# patterns=[x for x in a.split("\n\n")[0].split(", ")]
# designs=[x for x in a.split("\n\n")[1].splitlines()]
# patterns = set(patterns+patterns[::-1])
...
#             if design.startswith(current+pattern):
#                 queue.append(current+pattern)
#     if designPossible:
#         totalPossibleDesigns += 1
...
def build(pattern, towels):
    ...
    for t in towels:
        if pattern[:len(t)] == t:
            poss += build(pattern[len(t):], towels)
