...
# b=[[[z.split() for z in y.split(", ")] for y in x.split(": ")[1].split("; ")] for x in a.splitlines()]
# total=0
...
#         for z in y:
#             qty,colour = z
#             if maxColours[colour] < int(qty): 
#                 success=False
#     if success: total+= i+1
# print(total)
b=[[[z.split() for z in y.split(", ")] for y in x.split(": ")[1].split("; ")] for x in a.splitlines()]
total=0
for i,x in enumerate(b):
    maxColours={"red":0,"green":0,"blue":0}  
    for y in x:
        for z in y:
            qty,colour = z
            if maxColours[colour] < int(qty): 
                maxColours[colour] = int(qty)
    gameTotal = 1
    for y in maxColours.values(): gameTotal *= y
    total += gameTotal
print(total)