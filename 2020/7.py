a="""striped white bags contain 4 drab silver bags.
...
dic={}
for rule in b:
    bags=[(([int(rule[i-3])," ".join(rule[i-2:i])]) if rule[i-3].isnumeric() else 0) if i>2 else " ".join(rule[0:i]) for i,word in enumerate(rule) if word[:3] == "bag"]
    dic[bags[0]] = {x[1]:x[0] for x in bags[1:] if x!=0} 
for x in dic: print(x,dic[x])
...
def bfs(start):
    ...
    while queue:
        ...
        for neighbour in dic[node]:
            grandchildren[start].append(neighbour)
            queue.append(neighbour)

for start in dic:
    grandchildren[start] = []
    bfs(start)
    #print(start,grandchildren[start])

print(len([x for x in grandchildren if 'shiny gold' in grandchildren[x]]))#part1
...
def bfs2(start):
    ...
    while queue:
        node,qty=queue.pop()
        for neighbour in dic[node]:
            newQty = qty * dic[node][neighbour] 
            if neighbour not in  grandchildren[start]:   grandchildren[start][neighbour]=0
            if neighbour!=start: grandchildren[start][neighbour] += newQty
            queue.append([neighbour,newQty])
    total=0
    for x in grandchildren[start]: 
        qty=grandchildren[start][x]
        total+=qty
        print(x,qty)
    print(total)
#551 tooo low
