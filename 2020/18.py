a="""7 * (4 * 2 + 8 * (6 + 9) * 7 * 6) + (2 + 5 * 5 * 4 + 6 + 9) * 6
...
def getBracketPos(inp):
    ...
    for i,x in enumerate(inp):
        if x=="(" and b1==None: b1=i
        if x=="(": depth+=1
        if x==")": depth-=1
        if depth==0 and b1!=None:
            b2=i
            break
    return b1,b2


def solveBrackets(inp):
    b1,b2=getBracketPos(inp)
    return solve(inp[:b1]+solve(inp[b1+1:b2])+inp[b2+1:])

def solve(inp):
    if "(" in inp:
        inp = solveBrackets(inp)
    return solveWithoutBrackets(inp)

total=0
for line in a.splitlines():
    answer = int(solve(line))
    total += answer
    print(answer, "           ",line)
print(total)#part1
