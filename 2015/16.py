a="""Sue 1: goldfish: 6, trees: 9, akitas: 0
...
def isAunt(x):
    global present
    for thing,qty in x:
        if present[thing] != qty:
            return False
    return True

for i,x in enumerate(a.splitlines()):
    i+=1
    x = x[x.index(":")+2:]
    x = [[y.split(": ")[0],int(y.split(": ")[1])] for y in x.split(", ")]
    if not isAunt(x):
        continue
    print(i,x)
    break
