a="""turn on 489,959 through 759,964
...
for x in b:
    x1,y1 = [int(y) for y in x.split()[-3].split(",")]
    x2,y2 = [int(y) for y in x.split()[-1].split(",")]
    instr = x[:7]
    for j in range(y1,y2+1):
        for i in range(x1,x2+1):
            c[j][i] = 0 if instr=="turn of" else 1 if instr=="turn on" else 1-c[j][i]
print(sum([sum(y) for y in c])) #part 1
...
for x in b:
    ...
    for j in range(y1,y2+1):
        for i in range(x1,x2+1):
            c[j][i] += -1 if instr=="turn of" else 1 if instr=="turn on" else 2
            if c[j][i] < 0: c[j][i] = 0
print(sum([sum(y) for y in c])) #part 1